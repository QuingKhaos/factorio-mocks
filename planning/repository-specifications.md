...
│   ├── schema-generator.lua  # Generate JSON schemas from Factorio API docs
│   └── serialization.lua     # Serpent based serialization logic
```
...
- Distribute schemas as separate versioned ORAS artifacts (e.g., `ghcr.io/quingkhaos/factorio-json-schemas:2.0.x`)

**Serialization Strategy**:

- Use Serpent library for robust Lua table serialization
- Maintain human-readable format for debugging and inspection
- Include metadata with schema version references for validation
- Keep schemas separate to avoid duplication across data artifacts
...
│   │   │   ├── fr.cfg
│   │   │   └── [other-languages]/
│   │   └── metadata.json              # Includes schema version references
│   └── [other-modpacks]/
...
- JSON schemas generated by `factorio-mocks-generator` from Factorio's official API documentation
- Schemas distributed as separate versioned ORAS artifacts to avoid duplication
- Data metadata includes schema version references for validation
- Schema versioning aligned with Factorio API versions (2.0.x)
- Data validated at generation time using referenced schema versions
...
2. **Data Extraction & Schema Generation**: `factorio-mocks-generator` extracts data from modpack environments and generates
   validation schemas
3. **Data Publishing**: `factorio-mocks-generator` publishes extracted data and schemas to `factorio-mocks-data`
4. **Data Storage & Distribution**: `factorio-mocks-data` stores and distributes complete data artifacts
5. **Data Consumption**: `factorio-mocks-loader` fetches and provides data to consumer projects
